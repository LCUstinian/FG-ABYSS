...
// Tauri Mock 适配层 - 替换 Wails PayloadHandler
import { invoke } from '@/utils/tauri-mock-adapter'
import type { GeneratePayloadRequest, GeneratePayloadResponse, TemplateInfo } from '@/utils/tauri-mock-adapter'
...
const handleGenerate = async () => {
  try {
    ...
    const response = await invoke<GeneratePayloadResponse>('generate_payload', {
      type: formData.type,
...
const handleDeleteTemplate = async (row: TemplateInfo) => {
  try {
    const result = await invoke('delete_template', { templateName: row.name })
    if (result && result.success) {
...
const handleDeleteAllTemplates = async () => {
  try {
    ...
    for (const template of customTemplates.value) {
      try {
        await invoke('delete_template', { templateName: template.name })
      } catch (error) {
