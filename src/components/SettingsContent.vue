...
// Tauri Mock 适配层 - 替换 Wails SettingHandler
...
const changeAccentColor = async (color: string) => {
  ...
  try {
    // 写入数据库
    await invoke('save_setting', { key: 'accentColor', value: color })
    // 更新 CSS 变量
...
const changeFontFamily = async (font: string) => {
  ...
  try {
    // 写入数据库
    await invoke('save_setting', { key: 'fontFamily', value: font })
    // 更新 CSS 变量
...
const changeFontSize = async (size: string) => {
  ...
  try {
    // 写入数据库
    await invoke('save_setting', { key: 'fontSize', value: size })
    // 更新 CSS 变量
