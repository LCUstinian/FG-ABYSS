...
// Tauri Mock 适配层 - 替换 Wails DatabaseHandler
import { invoke } from '@/utils/tauri-mock-adapter'

// Mock 函数 - 替换 Wails bindings
const Connect = async (conn: any) => await invoke('database_connect', conn)
const Disconnect = async (connId: string) => await invoke('database_disconnect', { connectionId: connId })
const TestConnection = async (conn: any) => await invoke('database_test_connection', conn)
const ExecuteQuery = async (sql: string, connId: string) => await invoke('database_execute_query', { sql, connectionId: connId })
const GetTables = async (connId: string) => await invoke('database_get_tables', { connectionId: connId })
const GetTableColumns = async (tableName: string, connId: string) => await invoke('database_get_table_columns', { tableName, connectionId: connId })
